...
class Assets
  ...
  def combine(titles = [])
    ...
    builds(combine_titles).each do |key, build|
      ...
      if build['files'].size == 1
        source = File.read(build['files'][0]).to_s
        ...
        source = build['files'].map do |filepath|
          file_source_map = JSON.parse(File.read(filepath + '.map'))
          ...
          current_line += file.lines.count + 1
          file
        end.join("\n")
        source += "\n//# sourceMappingURL=#{build['path'].delete_prefix('public')}.map\n"
        File.write(build['path'] + '.map', source_map.to_json)
      end
  ...
  def compile_lib(output_name, name, *append_paths)
    ...
    if !@cache || !File.exist?(path)
      ...
      File.write("#{path}.map", builder.source_map.map.to_json)
  ...
  def compile(name, lib_path, ns = nil, game: nil)
    ...
    compilers.each do |compiler|
      ...
      File.write(opts[:js_path] + '.map', compiler.source_map.map.to_json)
    ...
    source = metadata.map do |_file, opts|
      ...
      source_map['sections'].append({
                                      ...
                                      'map' => JSON.parse(File.read(opts[:js_path] + '.map')),
                                    })
      current_line += file.lines.count + 1
      file
    end.join("\n")
    ...
    File.write(output + '.map', source_map.to_json)
    output
  end
